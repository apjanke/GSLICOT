...
C
C             K = 14*N*N + P*P + M*M + (6*N+4)*(P+M) + 3*P*M + 22*N +
C                 5*MIN(P,M) + 23.
C
C             The smallest workspace is obtained for DICO = 'C',
C             JOBE = 'I', and JOBD = 'Z', namely
C
C             K = MAX( 1, 2*N*N + 3*N*M + 2*N*P + P*M + 4*N + MIN(P,M) +
C                         MAX( P*M + c, 2*N*N + N*P + MAX(P,M) ) ).
C
C             for which a tight upper bound is
C
C             K = MAX( 1, 4*N*N + (3*N+1)*(P+M) + 2*P*M + 4*N +
C                         6*MIN(P,M) ).
C
...
C                   value;
C             = 1:  the matrix E or a pencil built by the algorithm is
C                   (numerically) singular;
C             = 2:  the QR (or QZ) algorithm for computing eigenvalues
C                   did not converge;
...
C     efficiency. Standard eigenvalue computations for Hamiltonian
C     matrices are used if JOBE = 'I', DICO = 'C', and the symmetric
C     matrices to be implicitly inverted are not too ill-conditioned.
C     Otherwise, generalized eigenvalue computations are used in the
C     iterative algorithm of [1].
C
C     REFERENCES
C
...
C     NUMERICAL ASPECTS
C
C     If the algorithm does not converge in MAXIT = 10 iterations
...
C     .. Local Scalars ..
     ...
     $                   MINCWR, MINPM, MINWRK, N2, N2PM, NEI, NWS, NX,
     $                   NY, PM, SDIM
     ...
     $                   RTOL, SAFMIN, SAFMAX, SMLNUM, TM, TOLER, WMAX,
     $                   WRMIN
C     ..
C     .. Local Arrays ..
...
C     .. External Functions ..
      ...
      EXTERNAL           AB13DX, DELCTG, DLAMCH, DLANGE, DLAPY2, LSAME
C     ..
C     .. External Subroutines ..
      EXTERNAL           DCOPY, DGEBAL, DGEHRD, DGEMM, DGEQRF, DGESVD,
     $                   DGGBAL, DGGES, DHGEQZ, DHSEQR, DLABAD, DLACPY,
     $                   DLASCL, DLASRT, DORGQR, DORMHR, DSWAP, DSYRK,
     $                   DTRCON, MA02AD, MB01SD, MB03SD, MB04ZD, TB01ID,
     $                   TG01AD, TG01BD, XERBLA
C     ..
...
C
     ...
     $            MINWRK = MAX( MINWRK, IR + 9*N )
               MINWRK = MAX( MINWRK, IR + 4*N + MAX( M, 2*N*N,
     $                                               N + IB + IE ) )
            ELSE
               MINWRK = MAX( MINWRK, IR + N + MAX( M, P, N*N + 2*N,
     $                                             3*N + IB + IE ) )
            END IF
            LW = 0
            IF ( .NOT.USEPEN ) THEN
     ...
     $                       MAX( N2PM + PM, 8*N*N + 14*N + 23 ) )
            MINWRK = MAX( 1, MINWRK, LW, IR + IA + IE )
         END IF
C
...
C
      ...
      IF ( WITHD ) THEN
         IS = ID + P*M
         IF ( FULLE .OR. DISCR .OR. NODYN ) THEN
            IU   = IS + MINPM
            IV   = IU
            IWRK = IV
            VECT = 'N'
...
C
      ...
      END IF
C
      IF ( .NOT.FULLE .AND. .NOT.DISCR .AND. WITHD ) THEN
C
...
C        Hessenberg form, and E upper triagular.
...
C        later use.
C        Additional workspace:  need   N*N + N;
C                               prefer larger.
C
         IAS  = IBT
         IWRK = IAS + N*N
...
C     Additional workspace:  need   2*N.
C
      IM    = IAS
      IAR   = IM + N
      IMIN  = II
...
C
     ...
     $                TM.LT.SAFMAX*DWORK( IBT+I ) ) ) THEN
                  ...
                  IF ( TM.NE.ZERO ) THEN
                     DWORK( II+I ) = ATAN2(
     $                                  DWORK( II+I ) / DWORK( IBT+I ),
     $                                  DWORK( IR+I ) / DWORK( IBT+I ) )
                     DWORK( IR+I ) = LOG( TM )
                  END IF
               ELSE
C
C                 Error return: matrix E is singular.
C
                  INFO = 1
                  RETURN
               END IF
...
C
            DO 90 I = 0, N - 1
               ...
               IF ( ( DWORK( IBT+I ).GE.ONE ) .OR.
     $              ( DWORK( IBT+I ).LT.ONE  .AND.
     $                MAX( TM, DWORK( IM ) ).LT.SAFMAX*DWORK( IBT+I ) ))
     $                   THEN
                  TM = TM / DWORK( IBT+I )
...
C
         GPEAK( 1 ) = ONE
         FPEAK( 1 ) = ABS( DWORK( IMIN ) )
         GPEAK( 2 ) = ZERO
         IF ( FPEAK( 1 ).LT.SAFMAX ) THEN
            FPEAK( 2 ) = ONE
         ELSE
            FPEAK( 1 ) = ONE
            FPEAK( 2 ) = ZERO
...
C           Additional workspace here:  need   6*N.
C
            NX   = N2
            II   = IR  + N2
            IBT  = II  + N2
...
C
     ...
     $                        ZERO, DWORK( IPA+N*N2 ), N2 )
               ELSE
                  CALL MA02AD( 'Full', N, N2, DWORK( IH12+PM*N2PM+N ),
     $                         N2PM, DWORK( IPA+N*N2 ), N2 )
                  NY = N
               END IF
            ELSE
               CALL DGEMM( 'Transpose', 'No Transpose', N2, N, P,
     ...
     $                     N2PM, B, LDB, ONE, DWORK( IPA+N*N2 ), N2 )
               NY = N2
            END IF
C
            IF ( FULLE ) THEN
     ...
     $                     ZERO, DWORK( IPE+N*N2 ), N2 )
               CALL DGEMM( 'Transpose', 'Transpose', N2, N, M,
     $                     BNORM/GAMMA, DWORK( IH12+PM*N2PM+N2 ), N2PM,
     $                     B, LDB, ONE, DWORK( IPE+N*N2 ), N2 )
            ELSE 
               IF ( FULLE )
     $            CALL DGEMM( 'Transpose', 'Transpose', N2, N, N, ONE,
     $                        DWORK( IH12+PM*N2PM+N ), N2PM, E, LDE,
...
C           Additional workspace: need   14*N+23;
C                                 prefer larger.
C
            CALL DGGES( 'No Vectors', 'No Vectors', 'No Sort', DELCTG,
     $                  N2, DWORK( IPA ), N2, DWORK( IPE ), N2, SDIM,
     $                  DWORK( IR ), DWORK( II ), DWORK( IBT ), DWORK,
     $                  N2, DWORK, N2, DWORK( IWRK ), LDWORK-IWRK+1,
     $                  BWORK, IERR )
            IF( IERR.GT.0 ) THEN
               INFO = 2
               RETURN
...
C           block of H.
C           Primary additional workspace: need   2*N*N+N
...
C           square-reduced method.
C           Additional workspace: need   2*N.
C
            IM   = IH
            IWRK = IH + ( N2 + 1 )*N
            CALL MB04ZD( 'No vectors', N, DWORK( IH ), N, DWORK( IH12 ),
     $                   N, DWORK, N, DWORK( IWRK ), IERR )
C
C           Additional workspace: need   (N+1)*N;
C                                 prefer larger.
C
            CALL MB03SD( 'Scale', N, DWORK( IH ), N, DWORK( IH12 ), N,
     $                   DWORK( IR ), DWORK( II ), DWORK( IWRK ),
     $                   LDWORK-IWRK+1, IERR )
            IF( IERR.GT.0 ) THEN
               INFO = 2
               RETURN
...
C
         IF ( USEPEN ) THEN
C
C           Additional workspace: need   2*N, if DICO = 'D';
C                                        0,   if DICO = 'C'.
C
            DO 260 I = 0, NX - 1
               TM = DLAPY2( DWORK( IR+I ), DWORK( II+I ) )
     ...
     $               TM.LT.SAFMAX*DWORK( IBT+I ) ) ) THEN
               ...
               ELSE
C
C                 Error return: the pencil is numerically singular.
C
                  INFO = 1
                  RETURN
               END IF
