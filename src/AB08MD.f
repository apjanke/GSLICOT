...
C     Workspace
C
C     IWORK   INTEGER array, dimension (2*N+MAX(M,P+1)+1)
C
...
C             LDWORK >= (N+P)*(N+M) + MAX( MIN(P+1,M) + MAX(3*M,N),
...
C     .. Local Scalars ..
      LOGICAL           LEQUIL
      INTEGER           I, KW, MU, NINFZ, NKROL, NM, NU, NP, RO, SIGMA,
     $                  WRKOPT
...
C
      ...
      ELSE IF( LDD.LT.MAX( 1, P ) ) THEN
         INFO = -12
      ELSE IF( LDWORK.LT.NP*NM + MAX( MIN( P+1, M ) + MAX( 3*M, N ),
     $                        MIN( P+1, N ) + MAX( 3*(P+1), NM ) ) )
     $      THEN
         INFO = -17
      END IF
C
...
C     Real workspace: need   (N+P)*(N+M) + MAX( MIN(P+1,M) + MAX(3*M,N),
C                            MIN(P+1,N) + MAX(3*(P+1),N+M) );
C                     prefer larger.
C     Integer workspace: 2*N+MAX(M,P+1)+1.
C
      RO = P
      SIGMA = 0
      NINFZ = 0
