...
C             not referenced.
C             On exit, if OMEGA = 0, DICO = 'C', B <> 0, and C <> 0, 
C             the leading N-by-N upper Hessenberg part of this array
C             contains the factors L and U from the LU factorization
C             of A (A = P*L*U); the unit diagonal elements of L are not
C             stored, L is lower bidiagonal, and P is stored in IWORK
C             (see SLICOT Library routine MB02SD).
...
C             On exit, if OMEGA = 0, DICO = 'C', B <> 0, C <> 0, and 
C             INFO = 0 or N+1, the leading N-by-M part of this array 
C             contains the solution of the system A*X = B.
C             Otherwise, this array is unchanged on exit.
C
C     LDB     INTEGER
...
C        Compute workspace.
C
         IF( SPECL .OR. ( NODYN .AND. WITHD ) ) THEN
...
C
         IF( LDWORK.LT.MINWRK ) THEN
            INFO = -20 
         ELSE
            IF ( NODYN .OR. ( OMEGA.EQ.ZERO .AND. .NOT.DISCR ) ) THEN
               MINCWR = 1
            ELSE
